...
mealPlanRouter
	.route('/getMealPlan')
	.get(authMiddleware, (req: RequestWithUserDecodedToken, res) => {
		const user = req.decoded!.user;

		MealPlan.findOne({ users: user._id })
			...
			.then((foundMealPlan) => {
				...
				mealPlanDocument
					...
					.catch((err) =>
						res
							.status(500)
							.json(new CustomError('Could not save week plan', err))
					);
			})
			.catch((err) =>
				res.status(500).json(new CustomError('Could not find week plan', err))
			);
	});

mealPlanRouter
	.route('/updateMealPlan')
	.put(authMiddleware, (req: RequestWithUserDecodedToken, res) => {
		...
		MealPlan.findByIdAndUpdate(mealPlanId, mealPlan, { new: true })
			...
			.catch((err) => {
				res
					.status(500)
					.json(
						new CustomError('Could not find week plan by id or update it', err)
					);
			});
	});
